...
#define GYRO_FAKTOR    (1 / (double)(0x7fff / 250.0))
...
public:
	void init();
	int testConnection();
	int readAccel(int16_t &ax, int16_t &ay, int16_t &az);
	int readGyro(float &gx, float &gy, float &gz);
	float readTemperature();
	void readAll(Attitude &envMess);

	void calibrate();											// Kalibriert den Sensor
	
	float gxCorrection;											// Korrektur-Faktor f�r die X-Achse des Gyroskops
	float gyCorrection;											// Korrektur-Faktor f�r die Y-Achse des Gyroskops
	float gzCorrection;											// Korrektur-Faktor f�r die Z-Achse des Gyroskops

private:
	int read(int start, uint8_t *buffer, int size);
	int write(int start, uint8_t *pData, int size);
	int write_reg(uint8_t reg, uint8_t data);
	float smooth(float data, float filterVal, float smoothedVal);

	bitConverter_t bitConverter;
	uint8_t buf[6];
};
